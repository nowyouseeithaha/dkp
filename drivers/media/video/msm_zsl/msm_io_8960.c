...
{
	uint32_t value = 0;
	value = msm_io_r(addr + offset);
//	CDBG("%s: Writing %x at (%x + %x), New value = %x\n",
//	       __func__, bitmask, addr, offset, value | bitmask);
	msm_io_w(value | bitmask , addr + offset);
}
...
static int msm_camera_vreg_enable(struct platform_device *pdev)
{
	...
	if (system_rev <= 1) {
		if (cam_vio == NULL) {
			...
			if (regulator_set_voltage(cam_vio, 1800000,
				1800000)) {
				CDBG("%s: VREG VIO set voltage failed\n",
					__func__);
				goto cam_vio_put;
			}
		...
		if (cam_vdig == NULL) {
			...
			if (regulator_set_optimum_mode(cam_vdig,
				CAM_VDIG_LOAD_UA) < 0) {
				CDBG("%s: VREG CAM VDIG set optimum"
				     " mode failed\n",
					__func__);
				goto cam_vdig_release;
			}
...
static void msm_camera_vreg_disable(void)
{
	...
	if (cam_vio) {
		regulator_set_voltage(cam_vio, 0, 1800000);
		regulator_disable(cam_vio);
		regulator_put(cam_vio);
		cam_vio = NULL;
	}
...
int msm_camio_sensor_clk_on(struct platform_device *pdev)
{
	...
	if (rc < 0) {
		pr_err("%s : retry gpio setting\n", __func__);
		rc = config_gpio_table(1);
		if (rc < 0) {
			pr_err("%s : error gpio setting\n", __func__);
			return rc;
		}
	}
	...
	if (!strcmp(sinfo->sensor_name, front_cam))
		...
		sinfo->sensor_platform_info->sensor_power_on(0, 0);
#endif
	...
	if (!strcmp(sinfo->sensor_name, front_cam))
		...
		sinfo->sensor_platform_info->sensor_power_on(0, 1);
#else
...
int msm_camio_sensor_clk_off(struct platform_device *pdev)
{
	...
	if (!strcmp(sinfo->sensor_name, front_cam))
		...
		sinfo->sensor_platform_info->sensor_power_off(0);
#else
